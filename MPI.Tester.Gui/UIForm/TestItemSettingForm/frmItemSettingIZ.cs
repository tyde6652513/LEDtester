...
{
    public partial class frmItemSettingIZ : Form, IConditionUICtrl, IConditionElecCtrl
    {
        ...
        private Dictionary<string, string> _dictIrItem;

        public frmItemSettingIZ()
        {
            InitializeComponent();

            this._lockObj = new object();

            this._item = new IZTestItem();
            ...
            this._dictIrItem = new Dictionary<string, string>();
        }

        public frmItemSettingIZ(TestItemDescription description) : this()
        {
        ...
        public Dictionary<string, string> IrItem
        {
            set { lock (this._lockObj) { this._dictIrItem = value; } }
        }
        #endregion
            ...
            foreach (var data in description.Property)
            {
                ...
                switch (keyName)
                {
                    ...
                    case EItemDescription.ForceValue:
                        {
                            this.dinForceValue.MaxValue = data.MaxValue * 1000;//ma -> uA
                            this.dinForceValue.MinValue = data.MinValue * 1000;//ma -> uA
        ...
        {
            chkIsFloatApplyValue.Checked = false;
            ...
            if (this._dictIrItem.Count > 0)
            {
                BindingSource bs = new BindingSource();

                bs.DataSource = this._dictIrItem;
        ...
        public bool CheckUI(out string msg)
        {
            ...
            if (this.chkIsFloatApplyValue.Checked)
            {
                if (this._dictIrItem.Count == 0)
                {
                    msg = "Can not find the Measured IR,\nplease cancel this item, and then set test item IR frist.";
        ...
        {
            this._item = (data as IZTestItem).Clone() as IZTestItem;
                ...
                if (DataCenter._conditionCtrl.Data.TestItemArray.Length > 0)
                {
                    foreach (var item in DataCenter._conditionCtrl.Data.TestItemArray)
                    {
                        ...
                        if (item.Order < data.Order )
                        {
                            ...
                            {
                                _dictIrItem.Add(item.MsrtResult[0].KeyName, item.MsrtResult[0].Name);
                            }
                        }
                        else
                        {
            ...
            if ((this._item as IZTestItem).RefIrKeyName != null)
            {
                if (this._dictIrItem.ContainsKey((this._item as IZTestItem).RefIrKeyName))
                {
                    this.cmbRefForceValue.SelectedValue = (this._item as IZTestItem).RefIrKeyName;
                }
            }
            this.chkIsFloatApplyValue.Checked = (this._item as IZTestItem).IsUseIrAsForceValue;

            this.dinFactor.Value = (this._item as IZTestItem).Factor;
            this.dinOffset.Value = (this._item as IZTestItem).Offset;
        ...
        {
            this._item.ElecSetting[0].ForceUnit = EAmpUnit.uA.ToString();

            this._item.ElecSetting[0].ForceDelayTime = (double)this.dinForceDelay.Value;

            this._item.ElecSetting[0].ForceValue = (double)this.dinForceValue.Value * (-1.0d);		// negative value
           ...
           // this._item.ElecSetting[0].ForceRangeIndex = this.cmbForceRange.SelectedIndex;
            ...
            //this._item.ElecSetting[0].MsrtRangeIndex = this.cmbMsrtRangeIZ.SelectedIndex;
            ...
            (this._item as IZTestItem).IsUseIrAsForceValue = this.chkIsFloatApplyValue.Checked;

            if (this._dictIrItem.Count > 0)
            {
                (this._item as IZTestItem).RefIrKeyName = this.cmbRefForceValue.SelectedValue.ToString();

                (this._item as IZTestItem).RefIrName = this._dictIrItem[(this._item as IZTestItem).RefIrKeyName];

            }

            (this._item as IZTestItem).Factor = this.dinFactor.Value;
            (this._item as IZTestItem).Offset = this.dinOffset.Value;
        ...
        private void chkIsFloatApplyValue_CheckedChanged(object sender, EventArgs e)
        {
            ...
            this.pnlFloatingIR.Visible = isVisible;
