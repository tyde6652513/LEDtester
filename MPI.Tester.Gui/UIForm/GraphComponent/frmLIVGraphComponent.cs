...
{
    public partial class frmLIVGraphComponent : Form
    {
        private LIVDataSet _dataSet;
        ...
        public frmLIVGraphComponent(ETestType testType) : this()
        {
        ...
        private void DrawCurveToGraph()
        {
            ...
            else
            {
                ...
                //    foreach (var keyName in this._dataSet.KeyNames)
                //    {
                //        if (count == 0)
                //        {
                //            this._xLabel = this.GetXAxisLabel(keyName, this._xAxisName);

                //            this._yLabel = this.GetYAxisLabel(this._curveName, this.cmbYAxis.SelectedItem as AxisMsrtItem);

                //            PlotGraph.SetLabel(this.zGraph, "", this._xLabel, this._yLabel, 14);
                //        }

                //        double[] xData = this.GetXAxisData(keyName, this._xAxisName);

                //        double[] yData = this.GetYAxisData(keyName, this.cmbYAxis.SelectedItem as AxisMsrtItem);

                //        if (xData != null && yData != null)
                //        {
                //            PlotGraph.DrawPlot(this.zGraph, xData, yData, true, 1.0F, this.GetColor(count), ZedGraph.SymbolType.None, false, true, true, keyName);
                //        }
        ...
        public void UpdateDataToControl(LIVDataSet data)
        {
            ...
            {
                LIVData item = this._dataSet[i];

                GraphCurveItem newCurveItem = null;

                if (this._displayTestType == ETestType.LIV && item.KeyName.Contains(ETestType.LIV.ToString()))
                {
                    newCurveItem = new GraphCurveItem(item.KeyName, item.Name);
                }
                else if (this._displayTestType == ETestType.TRANSISTOR && item.KeyName.Contains(ETestType.TRANSISTOR.ToString()))
                {
                    newCurveItem = new GraphCurveItem(item.KeyName, item.Name);
                }
                ...
                if (newCurveItem != null)
                {
                    ...
                    foreach (var result in item)
                    {
                        ...
                        else
                        {
							...
							if (result.KeyName.Contains("TRMsrtDrain") || result.KeyName.Contains("TRMsrtSource") ||
								result.KeyName.Contains("TRMsrtGate") || result.KeyName.Contains("TRMsrtBluk"))
							{
								// Update X Axis
								newCurveItem.AxisItemsX.Add(new AxisMsrtItem(result.KeyName, result.Name, result.Unit));
							}
        ...
        private void frmLIVGraphComponent_VisibleChanged(object sender, EventArgs e)
        {
            if (this.Visible)
            {
            	this.DrawCurveToGraph();
