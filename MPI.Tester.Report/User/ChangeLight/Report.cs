...
namespace MPI.Tester.Report.User.ChangeLight
{
	class Report : ReportBase
	{
        ...
        private Dictionary<string, double> _esdResultKeyNameAndTestCount;// HBM 2K Pass=1數量/(應打HBM總數 - IR0 Fail)
		...
		protected override void SetResultTitle()
		{
            ...
            this._esdResultKeyNameAndTestCount.Add(EESDMode.HBM.ToString(), 0);

            this._esdResultKeyNameAndTestCount.Add(EESDMode.MM.ToString(), 0);
		...
		protected override EErrorCode WriteReportHeadByUser()
		{
			...
			string pitch = "XPitch:" + (this.UISetting.XPitch1 * 1000).ToString("0") + ", " + (this.UISetting.XPitch2 * 1000).ToString("0") + ", " + (this.UISetting.XPitch3 * 1000).ToString("0") + ", " + (this.UISetting.XPitch4 * 1000).ToString("0") + ", " + (this.UISetting.XPitch5 * 1000).ToString("0");

			pitch += "; YPitch:" + (this.UISetting.YPitch1 * 1000).ToString("0") + ", " + (this.UISetting.YPitch2 * 1000).ToString("0") + ", " + (this.UISetting.YPitch3 * 1000).ToString("0") + ", " + (this.UISetting.YPitch4 * 1000).ToString("0") + ", " + (this.UISetting.YPitch5 * 1000).ToString("0");
            ...
            if (this.UISetting.MESMachineName == Data.EEqModelName.P7602)
            {
                this.WriteLine("Operator," + this.UISetting.WeiminUIData.CustomerNote03);
            }
            else
            {
			...
			this.WriteLine("Test Condition,\"" + this.GetData("testCondition") + ";" + pitch + "\"");
		...
		protected override EErrorCode PushDataByUser(Dictionary<string, double> data)
        {
            ...
            if (isHBMIsTester)
            {
                this._esdResultKeyNameAndTestCount[EESDMode.HBM.ToString()]++;
            }
            else if (isMMIsTester)
            {
                this._esdResultKeyNameAndTestCount[EESDMode.MM.ToString()]++;
            }
		...
		{
			if(this.UISetting.WeiminUIData.WMTestMode != 1)
			{
				return EErrorCode.NONE;
			}
            ...
            if (this.UISetting.MESMachineName == Data.EEqModelName.P7602)
            {
                this.WriteLine2("Operator," + this.UISetting.WeiminUIData.CustomerNote03);
            }
            else
            {
			...
			this.WriteLine2("Test Condition,\"" + this.GetData("testCondition") + ";" + pitch + "\"");
		...
		protected override EErrorCode RewriteReportByUser2()
		{
			...
			foreach (var esdKeyName in this._esdTestItem)
			{
				string resultKeyName = esdKeyName.Key.Replace("ESD", "MIRJ");
                ...
                if (this.Product != null && this.Product.TestCondition != null && this.Product.TestCondition.TestItemArray != null)
                {
                    foreach (var item in this.Product.TestCondition.TestItemArray)
                    {
                        if (item.KeyName == esdKeyName.Key)
                        {                            
                            if (this._esdResultKeyNameAndPassDie.ContainsKey(resultKeyName))
                            {
                                if (this._esdResultKeyNameAndTestCount[(item as ESDTestItem).EsdSetting.Mode.ToString()] == 0)
                                {
                                    this._esdResultKeyNameAndTestCount[(item as ESDTestItem).EsdSetting.Mode.ToString()] = 1;
                                }

                                double yield = this._esdResultKeyNameAndPassDie[resultKeyName] / this._esdResultKeyNameAndTestCount[(item as ESDTestItem).EsdSetting.Mode.ToString()] * 100.0d;
					...
					foreach (var item in this.Product.TestCondition.TestItemArray)
					{
                        if (item is ESDTestItem)
                        {
                            ...
                            if (esdItem.Key == item.KeyName && item.IsEnable)
                            {
                                ...
                                if (this._esdResultKeyNameAndPassDie.ContainsKey(resultKeyName))
                                {
                                    double esdTestCount = 1.0d;

                                    if ((item as ESDTestItem).EsdSetting.Mode == EESDMode.HBM)
                                    {
                                        esdTestCount = this._esdResultKeyNameAndTestCount[EESDMode.HBM.ToString()];
                                    }
                                    else
                                    {
                                        esdTestCount = this._esdResultKeyNameAndTestCount[EESDMode.MM.ToString()];
                                    }
                                    ...
                                    double yield = this._esdResultKeyNameAndPassDie[resultKeyName] / esdTestCount * 100.0d;
			...
			foreach (var item in this._esdTestItem)
			{
                ...
                foreach (var result in this.AcquireData.OutputTestResult)
				{
                    if (this._esdResultKeyNameAndZapValue.ContainsKey(key) && key == result.KeyName)
					{
                        ...
                        string judgeIR = data[key].ToString(format);

                        string isPass = result.IsPassAndIsTested ? "1" : "0";
        ...
        {
			if (!this.UISetting.IsEnableRunMesSystem)
			{
				this.UISetting.WeiminUIData.OutputFileName = this.UISetting.Barcode;
			}
			...
			if (this.UISetting.WeiminUIData.WMTestMode == 0 || this.UISetting.WeiminUIData.WMTestMode == 3)
            {
                fileName = this.UISetting.WeiminUIData.OutputFileName + "_COT";
            }
			else if (this.UISetting.WeiminUIData.WMTestMode == 1)
            {
				fileName = this.UISetting.WeiminUIData.OutputFileName.Replace(this.UISetting.WeiminUIData.CustomerNote02, "") + "_COW";
            }
            else
            {
