...
namespace MPI.Tester.Device.ESD
{
    ...
    internal class UniDAQ
    {
        ...
        public const UInt16 IXUD_ACTIVE_HIGH = ((ushort)(4));  // Active High Trigger
        public const UInt16 IXUD_ACTIVE_LOW = ((ushort)(8));   // Active Low Trigger
        ...
        [DllImport(@"C:\MPI\LEDTester\Driver\UniDAQ.dll")]
        ...
        [DllImport(@"C:\MPI\LEDTester\Driver\UniDAQ.dll")]
        public static extern UInt16 Ixud_ReadDI32(UInt16 wBoardNo, UInt16 wPortNo, ref UInt32 dwDILow, ref UInt32 dwDIHigh);

        [DllImport(@"C:\MPI\LEDTester\Driver\UniDAQ.dll")]
        public static extern UInt16 Ixud_WriteDO32(UInt16 wBoardNo, UInt16 wPortNo, UInt32 dwDOLow, UInt32 dwDOHigh);

        [DllImport(@"C:\MPI\LEDTester\Driver\UniDAQ.dll")]
        public static extern UInt16 Ixud_ReadBackDO32(UInt16 wBoardNo, UInt16 wPortNo, ref UInt32 dwLow, ref UInt32 dwHigh);
    ...
    internal class PISODA2UWrapper
    {
        private object _lockObj;

        private const int _hwCardID = 12;
        ...
        public bool Init()
        {
            ...
            ret = UniDAQ.Ixud_SearchCard(ref this._totalBoard, UniDAQ.PISODA2);
            if (ret != UniDAQ.Ixud_NoErr || this._totalBoard == 0 )
                return false;

            this._modeName = new byte[20];
            for ( index = 0; index < this._totalBoard; index++)
            {
                ret = UniDAQ.Ixud_GetCardInfo(index, ref  sDeviceInfo[index], ref sCardInfo[index], this._modeName);

                if (ret != UniDAQ.Ixud_NoErr)
                    return false;

                if (sCardInfo[index].dwModelNo == UniDAQ.PISODA2 &&
                     sCardInfo[index].CardID == _hwCardID)
                {
                    this._boardID = index;
            ...
            {
                ret += UniDAQ.Ixud_ConfigAO(this._boardID, i, UniDAQ.IXUD_AO_UNI_5V);
                ret += UniDAQ.Ixud_WriteAOVoltage(this._boardID, i, 0.0f);
            }
        ...
        public void Close()
        {
            for (ushort i = 0; i < this._totalChannel; i++)
            {
                UniDAQ.Ixud_WriteAOVoltage(this._boardID, i, 0.0f);
            }
            UniDAQ.Ixud_DriverClose();
            this._isInitOK = false;
        }

        public void DAOutput(int channel, double value)
        {
            if (this._isInitOK == false || this._boardID < 0 )
                return;

            UniDAQ.Ixud_WriteAOVoltage(this._boardID, (ushort)channel, Convert.ToSingle(value));
        }
