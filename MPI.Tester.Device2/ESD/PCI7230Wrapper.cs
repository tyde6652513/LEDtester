...
namespace MPI.Tester.Device.ESD
{
    ...
    //------------------------------------------------------------------
	    [DllImport(@"C:\MPI\LEDTester\Driver\PCI-Dask.dll")]
    ...
    public class PCI7230Wrapper
    {
        private object _lockObj;

        private short _deviceNum;
        private byte[] _outByteArray;
        private bool _isInitOK;

        public PCI7230Wrapper()
        {
            this._lockObj = new object();

            this._deviceNum = -1;
            this._outByteArray = new byte[2];
            this._isInitOK = false;
        }

        #region >>> Private Methods <<<

        private short portOutput(int portNum, uint outData)
        {
            if (this._deviceNum < 0)
                return -1234;

            return DASK.DO_WritePort( (ushort)this._deviceNum, (byte)portNum, (uint)outData);
        }
        ...
        public bool Init()
        {
            if (System.IO.File.Exists(@"C:\MPI\LEDTester\Driver\PCI-Dask.dll") == false)
            {
                return false;              
            }

            this._deviceNum = DASK.Register_Card(DASK.PCI_7230, 0);
            ...
            this.portOutput(0, 0x00);
            this.portOutput(1, 0x00);
            this._isInitOK = true;
        ...
        public void Close()
        {
            ...
            if (this._deviceNum >= 0)
            {
                ret = DASK.Release_Card( (ushort) this._deviceNum);
            }
        ...
        public void BitOutput(int bitPos, int active)
        {
            ...
            if (this._isInitOK == false || this._deviceNum < 0 )
                return;
           ...
           outData = (uint)this._outByteArray[0] + ( (uint)this._outByteArray[1] << 8 );
           this.portOutput(0, outData);


           
        }

        public bool BitInput(bool[] BitArray)
        {
            if (this._isInitOK == false || this._deviceNum < 0)
                return false;
            ...
            uint value;
            DASK.DI_ReadPort((ushort)this._deviceNum, 0, out value);
        ...
        public bool BitInput(out uint value)
        {
			value = 0;
