...
{
	public class HR4000 : ISpectroMeter
	{
		...
		private const int MIN_INTEGRATION_TIME = 1;
		private const int MAX_INTEGRATION_TIME = 650;
		private const int DARK_COUNT = 2000; //25000;
		private const int MAX_COUNT = 16000; //64000;
		private const int MIN_WAVELENGTH = 350;
		private const int MAX_WAVELENGTH = 1000;
		private const int MAX_ADC_COUNT = 16383; //65535;


		/*private const string CAL_FILE_EXT = ".mcal";

		private const string CAL_TRANSMISSIOM_EXT = ".transmission";*/


		private object _lockObj;

        private bool _isExtTrigStatus = false;
		...
		//private int _modifiedmaxCount = 79000;
        ...
        private int _statusSptMode = 0xFF;
        private int _statusIntegrationTime_us = 0;

		public HR4000( SpectrometerHWSetting HWSetting )
		{
			this._lockObj = new object();

			this._devSetting = new OptiDevSetting();
			this._oceanWrapper = new SeaBreezeWrapper( ESpectrometerModel.HR4000 );
            ...
            this._wavelengthArray = new double[this._oceanWrapper.CCD_PIXEL_LENGTH];
            this._recordIntensityArray = new double[1][] { new double[this._oceanWrapper.CCD_PIXEL_LENGTH] };
            this._currentIntensityArray = new double[this._oceanWrapper.CCD_PIXEL_LENGTH];
            this._darkIntensityArray = new double[this._oceanWrapper.CCD_PIXEL_LENGTH];
            ...
            switch ( mode )
			{
				case 0:
                   ...
                   // setMode = 0x00;
                    ...
                    if (_statusSptMode != setMode)
                    {
                        this._oceanWrapper.setExternalTriggerMode(0, setMode);			//Self-Hardware Trigger mode 0x83
                        //this._oceanWrapper.setExternalTriggerMode( 0, setMode );		//Free Running mode 0x00
                        //this._oceanWrapper.setExternalTriggerMode( 0, setMode );			//Shutter mode  0x03

                        _statusSptMode = setMode;
                    }
					break;
				//------------------------------------------------------------------------------
				case 1:
                    ...
                    if (_statusSptMode != setMode)
                    {
                        this._oceanWrapper.setExternalTriggerMode(0, setMode);
		...
		/*public bool LoadCSVFormatFile( string seriesNum )
		{
			try
			{
				string CaliFilePath = @"C:\MPI\LEDTester\Spectrometer\";

				string CsvCalibrationFile = Path.Combine( CaliFilePath, seriesNum + CAL_FILE_EXT );

				string CsvTransmissionFile = Path.Combine( CaliFilePath, seriesNum + CAL_TRANSMISSIOM_EXT );

				if ( !File.Exists( CsvCalibrationFile ) )
				{
					return false;
				}

				SpectrumDataCtrl.SpectrumSet calbSpectrum = SpectrumDataCtrl.LoadFile( CsvCalibrationFile, "Wavelength,ratio", ',' );

				if ( !File.Exists( CsvTransmissionFile ) )
				{
					return false;
				}

				SpectrumDataCtrl.TransmissionSpectrum transSpectrum = SpectrumDataCtrl.LoadTransmissionFile( CsvTransmissionFile, "Wavelength,ND1,ND2,ND3,ND4,ND5", ',' );

				this._spectroCaliData = new SpectroCaliData2();

				this._spectroCaliData.SeriesNum = seriesNum;

				this._spectroCaliData.MWCoeff = 1.0d;

				this._spectroCaliData.LightPowerFactor = calbSpectrum.lightPowerfactor;

				this._spectroCaliData.CalibrationDate = DateTime.Now.ToString();

				for ( int i = 0; i < transSpectrum.ND1.Length; i++ )
				{
					this._spectroCaliData.CaliSpectrumArray[ 0 ][ i ] = transSpectrum.ND1[ i ] * calbSpectrum.absoluate[ i ];

					this._spectroCaliData.CaliSpectrumArray[ 1 ][ i ] = transSpectrum.ND2[ i ] * calbSpectrum.absoluate[ i ];

					this._spectroCaliData.CaliSpectrumArray[ 2 ][ i ] = transSpectrum.ND3[ i ] * calbSpectrum.absoluate[ i ];

					this._spectroCaliData.CaliSpectrumArray[ 3 ][ i ] = transSpectrum.ND4[ i ] * calbSpectrum.absoluate[ i ];

					this._spectroCaliData.CaliSpectrumArray[ 4 ][ i ] = transSpectrum.ND5[ i ] * calbSpectrum.absoluate[ i ];
				}
			...
			{
				Console.WriteLine( "[HR4000], LoadCSVFormatFile() , Load Fail " );
		...
		{
			Console.WriteLine( "[HR4000], LoadBinaryCaliFile()" );
			...
			if ( File.Exists( pathAndFile ) )
			{
				...
				{
					this._spectroCaliData = new SpectroCaliData(this._oceanWrapper.CCD_PIXEL_LENGTH);
		...
		/*private bool GetWavelengthByCoefficent( string seriesNum )
		{
			bool isEnable = this._devSetting.IsEnableLoadWavelenghtCoefficent || this._hardwareSetting.IsUseOSRAMCalibFormat;
			...
			string fileAndPath = Path.Combine( CaliFilePath, seriesNum + ".wlc" );
			...
			double[] tempWavelength = new double[ CCD3648_PIXEL_LENGTH ];

			List<string[]> coeffdata = CSVUtil.ReadCSV( fileAndPath );

			double intercept = 0.0d;

			double firstCoefficient = 0.0d;

			double secondCoefficient = 0.0d;

			double thirdCoefficient = 0.0d;

			try
			{
				double.TryParse( coeffdata[ 1 ][ 1 ], out intercept );

				double.TryParse( coeffdata[ 2 ][ 1 ], out firstCoefficient );

				double.TryParse( coeffdata[ 3 ][ 1 ], out secondCoefficient );

				double.TryParse( coeffdata[ 4 ][ 1 ], out thirdCoefficient );

				string para = intercept.ToString() + "," + firstCoefficient.ToString() + "," + secondCoefficient.ToString() + "," + thirdCoefficient.ToString();

				Console.WriteLine( "[HR4000], Load Wavelength Coefficent Ok , parameter " + para );
			}
			catch
			{
				Console.WriteLine( "[HR4000], Load Wavelength Coefficent Error" );
			}

			for ( int p = 0; p < CCD3648_PIXEL_LENGTH; p++ )
			{
				tempWavelength[ p ] = intercept + ( firstCoefficient * p ) + secondCoefficient * Math.Pow( p, 2 ) + thirdCoefficient * Math.Pow( p, 3 );
			}

			if ( tempWavelength[ 0 ] < 200 )
			{
				this._wavelengthArray = this._oceanWrapper.getWavelengths( 0 );
		...
		private bool MeterTrigger( double IntegralTime )
		{
			...
			{
				//-----------------------------------------------------------------------------------
				// (1) Set integration time to HR4000
				//-----------------------------------------------------------------------------------

                int intTime = (int)(1000 * IntegralTime);

                if (intTime != _statusIntegrationTime_us)
                {
                    this._oceanWrapper.setIntegrationTime(0, (int)(1000 * IntegralTime));

                    _statusIntegrationTime_us = intTime;
                }
				//-----------------------------------------------------------------------------------
				// (2) Get  intensity array from HR4000 , CCD response
				//-----------------------------------------------------------------------------------
		...
		{
			Console.WriteLine( "[HR4000], Init() , Start Initial" );
			...
			try
			{
				...
				Console.WriteLine( "[HR4000], Init(), serialNum = " + this._serialNum );
				if ( this._serialNum == string.Empty )
				{
					this._errorNum = EDevErrorNumber.SpectrometerDevice_Init_Err;
					Console.WriteLine( "[HR4000], Init() , EDevErrorNumber.SpectrometerDevice_Init_Err, SN = Empty" );

					return false;
				}

                if (!this._serialNum.Contains("HR4"))
                {
                    Console.WriteLine("[HR4000], Init() , Fail! Wrong device model, SN = " + this._serialNum);
                    this._errorNum = EDevErrorNumber.SpectrometerDevice_Init_Err;
                    return false;
                }
				//
				...
				Console.WriteLine( "[HR4000], Init() , Success, SN = " + this._serialNum );
				Console.WriteLine( "[HR4000], Init() ,getNumberOfSpectrometersFound =" + numSpectrometer.ToString() );

				this._wavelengthArray = this._oceanWrapper.getWavelengths( 0 );
				//this._serialNum = this._oceanWrapper.getSerialNumber();
				this._version = this._oceanWrapper.getApiVersion();
				this._wavelengthArray = ( double[] ) this._oceanWrapper.getWavelengths( 0 );
				//this._modifiedmaxCount = ( int ) this._oceanWrapper.GetCompensatedMaxCount();
				//this._modifyNonlinearCountRate = ( double ) MAX_COUNT / _modifiedmaxCount;				
			}
			catch
			{
				this._errorNum = EDevErrorNumber.SpectrometerDevice_Init_Err;
				Console.WriteLine( "[HR4000], Init() , EDevErrorNumber.SpectrometerDevice_Init_Err" );
				return false;
			}
			...
			if ( this.LoadBinaryCaliFile( this._serialNum ) == false )
			{
				...
				Console.WriteLine( "[HR4000], Init() , EDevErrorNumber.LoadCalibrationFileFail" );
				return false;
			}

			/*if ( this._hardwareSetting.IsUseOSRAMCalibFormat )
			{
				//  this._devSetting.IsEnableLoadWavelenghtCoefficent = true;

				if ( !this.LoadCSVFormatFile( this._serialNum ) )
				{
					string CaliFilePath = @"C:\MPI\LEDTester\Spectrometer\";

					string CsvCalibrationFile = Path.Combine( CaliFilePath, this._serialNum + CAL_FILE_EXT );

					string CsvTransmissionFile = Path.Combine( CaliFilePath, this._serialNum + CAL_TRANSMISSIOM_EXT );
			...
			if ( GetWavelengthByCoefficent( this._serialNum ) == false )
			{
				this._errorNum = EDevErrorNumber.SpectrometerLoadWavelengthPara_Err;
			...
			if ( this._SPAM.Init() == false )
			{
				...
				Console.WriteLine( "[HR4000], Init() , EDevErrorNumber.SPAMDriver_Init_Err" );
				return false;
			}

			this._errorNum = EDevErrorNumber.Device_NO_Error;
			// Console.WriteLine("[HR4000], Init() , EDevErrorNumber.Success");
			return true;
		}
		...
		{
			// Console.WriteLine("[HR4000], SetConfigToMeter() ");

			//if (devSetting.LimitStartTime < 6)
			//	{
			//	devSetting.LimitStartTime = 6;
			//	}
			...
			devSetting.IsCorrectForNonlinearity = true; // false for test
			...
			//this._devSetting.IsEnableHardwareTrigger = true;
           // this.SetTriggerMode(0);
			...
			if ( this._devSetting.IsCorrectForNonlinearity == true )
			{
				...
				Console.WriteLine( "Enable Correct For Detector Nonlinearity" );
			...
			else
			{
				...
				Console.WriteLine( "Disable Correct For Detector Nonlinearity" );
			}
			...
			if ( this._wavelengthArray != null )
			{
				...
				Console.WriteLine( "[HR4000],SPT CalcPixel(), wl start=" + this._devSetting.StartWavelength.ToString() + ", End=" + this._devSetting.EndWavelength.ToString() );
			}
			else
			{
				Console.WriteLine( "[HR4000], SPT CalcPixel(), wl = null" );
			}
			//-----------------------------------------------------------------------------------
			...
			if ( this._spectroCaliData != null )
			{
				if ( this._devSetting.IsUseNDFilterRatio == true && this._spectroCaliData.BaseCalibSpectrum != null )
				{
                    ...
                    double[] ratio = new double[this._oceanWrapper.CCD_PIXEL_LENGTH];
                    this._caliSpectrum = new double[this._oceanWrapper.CCD_PIXEL_LENGTH];

                    for (int i = 0; i < this._oceanWrapper.CCD_PIXEL_LENGTH; i++)
					{
			...
			if ( this._devSetting.IsUseProductXaxisCoeff == true )
			{
				...
				{
                    double[] tempWavelength = new double[this._oceanWrapper.CCD_PIXEL_LENGTH];
                    ...
                    for (int p = 0; p < this._oceanWrapper.CCD_PIXEL_LENGTH; p++)
					{
						tempWavelength[ p ] = intercept + ( firstCoefficient * p ) + secondCoefficient * Math.Pow( p, 2 ) + thirdCoefficient * Math.Pow( p, 3 );
					}
					this._wavelengthArray = tempWavelength;
			...
			/*if ( GetWavelengthByCoefficent( this._serialNum ) == false )
			{
				...
				Console.WriteLine( "[HR4000], SetData() , EDevErrorNumber.LoadCalibrationFileFail" );
			...
			{
                this._calibWeightArray = new double[this._oceanWrapper.CCD_PIXEL_LENGTH];
		...
		{
			//Console.WriteLine("[HR4000], SetParamToMeter() ");
			int settingCount = 1;
			...
			{
				Console.WriteLine( "[HR4000], SetParamToMeter(), LOPWL_" + i.ToString() + " - " + paramSetting[ i ].SensingMode + " Fix=" + paramSetting[ i ].FixIntegralTime + ", Limit=" + paramSetting[ i ].LimitIntegralTime );
				this._opticalData[ i ] = new OptiData();
				this._recordIntTime[ i ] = this._devSetting.LimitStartTime;
				this._calculatetIntTime[ i ] = this._devSetting.LimitStartTime;

                this._recordIntensityArray[i] = new double[this._oceanWrapper.CCD_PIXEL_LENGTH];
                this._absoluteSepctrum[i] = new double[this._oceanWrapper.CCD_PIXEL_LENGTH];
		...
		public bool CalculateParameters( uint index )
		{
			...
			{
                this._absoluteSepctrum[index] = new double[this._oceanWrapper.CCD_PIXEL_LENGTH];
				return true;
			}
			...
			if ( this._isFileterAbsSpectrum )
			{
				...
				int indexPeak = MpiSPAM.CalcMaxCount( this._absoluteSepctrum[ index ], 0, 0 );

				double PeakWave = this._wavelengthArray[ indexPeak ];

				double bn = this._devSetting.BaseNoise;

				if ( PeakWave > this._devSetting.BaseCuttingWave )
				{
					bn = this._devSetting.BaseNosieTwo;
				}
				else
				{
					bn = this._devSetting.BaseNoise;
				}

				double cutValue = 100 / bn;

				//   Console.WriteLine("[HR4000], _isFileterAbsSpectrum : " + cutValue.ToString());
				
				this._absoluteSepctrum[ index ] = MpiSPAM.FilterData( this._wavelengthArray, 555, this._absoluteSepctrum[ index ], cutValue );*/
			}
			...
			else
			{
				...
				//this._mpiSPAM2.CaculateParameter( ref optDataPackage, this._wavelengthArray, this._absoluteSepctrum[ index ], 200, 1000 );
			...
			/*if ( this._opticalData[ index ].WLD > 1000 )
			{
				this._opticalData[ index ].WLD = 999;
			}

			this._opticalData[ index ].WLD = this._opticalData[ index ].WLD + this._spectroCaliData.LamdaDOffset;

			this._opticalData[ index ].WLP = this._opticalData[ index ].WLP + this._spectroCaliData.WavelengthOffset;*/
		...
		public double[] GetDarkSample( uint samplingDarkCount, uint IntTime )
		{
            ...
            if (_statusIntegrationTime_us != IntTime)
            {
                this._oceanWrapper.setIntegrationTime(0, (int)IntTime);  // unit: us

                _statusIntegrationTime_us = (int)IntTime;

                System.Threading.Thread.Sleep(100);
            }

            double[] darkSpectrumSum = new double[this._oceanWrapper.CCD_PIXEL_LENGTH];
			...
			//    this._darkIntensityArray = this._oceanWrapper.getSpectrum( 0 );
			...
			for ( int i = 0; i < samplingDarkCount; i++ )
			{
                ...
                for (int pix = 0; pix < this._oceanWrapper.CCD_PIXEL_LENGTH; pix++)
				{
