...
{
	public class USB2000P : ISpectroMeter
	{
        ...
        public USB2000P(SpectrometerHWSetting HWSetting)
		{
			this._lockObj = new object();

			this._devSetting = new OptiDevSetting();
			this._oceanWrapper = new SeaBreezeWrapper();
		...
		{
            Console.WriteLine("[USB2000+], Init() , Start Initial");
			...
			try
			{
                ...
                Console.WriteLine("[USB2000+], Init() ,getNumberOfSpectrometersFound =" + numSpectrometer.ToString());

				this._wavelengthArray = this._oceanWrapper.getWavelengths(0);
				this._serialNum = this._oceanWrapper.getSerialNumber();

                if (!this._serialNum.Contains("USB"))
                {
                    Console.WriteLine("[USB2000+], Init() , Fail! Wrong device model, SN = " + this._serialNum);
			...
			{
				this._errorNum = EDevErrorNumber.SpectrometerDevice_Init_Err;
                Console.WriteLine("[USB2000+], Init() , EDevErrorNumber.SpectrometerDevice_Init_Err");
				return false;
			}
            ...
            if (this.LoadBinaryCaliFile(this._serialNum) == false)
			{
                ...
                Console.WriteLine("[USB2000+], Init() , EDevErrorNumber.LoadCalibrationFileFail");
				return false;
			}
			...
			if (this._SPAM.Init() == false)
			{
                ...
                Console.WriteLine("[USB2000+], Init() , EDevErrorNumber.SPAMDriver_Init_Err");
				return false;
			}

			this._errorNum = EDevErrorNumber.Device_NO_Error;
           // Console.WriteLine("[USB2000+], Init() , EDevErrorNumber.Success");
			return true;
		}
		...
		{
           // Console.WriteLine("[USB2000+], SetConfigToMeter() ");
		...
		{
            //Console.WriteLine("[USB2000+], SetParamToMeter() ");
			int settingCount = 1;
		...
		public bool CalculateParameters(uint index)
		{
            ...
            else
            {
                ...
                this._mpiSPAM2.CaculateParameter(ref optDataPackage, this._wavelengthArray, this._absoluteSepctrum[index], 380, 780);
